...
```bash
  cd .\.ssh\
  ssh -i "TargetKeyGen1.pem" ec2-user@ec2-15-188-80-96.eu-west-3.compute.amazonaws.com
```
...
se connecter a l'API héberger sur EC2 : http://15.188.80.96:5203/swagger => a chaque terraform apply l'ip put change
Lien du site depuis l'EC2 : http://15.188.80.96/auth => a chaque terraform apply l'ip peut change
